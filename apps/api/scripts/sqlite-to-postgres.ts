...
async function main() {
  if (!fs.existsSync(sqlitePath)) {
    throw new Error(`[migrate] sqlite file not found: ${sqlitePath}`);
  }
  ...
  try {
    ...
    if (dryRun) {
      ...
      console.log(JSON.stringify({ sqlitePath, ...counts }, null, 2));
      return;
    }

    if (!databaseUrl) {
      throw new Error("[migrate] DATABASE_URL is required unless --dry-run is used");
    }
